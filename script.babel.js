      ...
      try {
        form.addEventListener('submit', function (e) {
          ...
          if (formPrivacy) {
            ...
            if (!formPrivacyInput_checked) {
              ...
              setTimeout(() => {
                formPrivacy.classList.remove('--shake', '--warning');
              }, 1000);
              return;
            }
          }
          ...
          if (form__name && !data.name) {
            ...
            setTimeout(() => {
              form__name.classList.remove('--shake', '--warning');
            }, 1000);
            return;
          }
          ...
          if (form__email && !data.email) {
            ...
            setTimeout(() => {
              form__email.classList.remove('--shake', '--warning');
            }, 1000);
            return;
          }
          ...
          if (form__phone && !data.phone) {
            ...
            setTimeout(() => {
              form__phone.classList.remove('--shake', '--warning');
            }, 1000);
            return;
          }
        ...
        function showResultSendingForm(jsonResponse) {
          ...
          if (jsonResponse.ok) {
            ...
            mailSendIndicatorImg.setAttribute('src', '/images/animations/mail_sent/lime_200_once.gif'); // mailSendIndicatorImg.setAttribute('rel', 'preload');

            mailSendIndicator.append(mailSendIndicatorImg);
            form.append(mailSendIndicator); // const mailSendIndicator = document.querySelector('.form__send-indicator');
            ...
            const mailSendIndicator = document.createElement('div');
            const mailSendIndicatorImg = document.createElement('img');
            mailSendIndicatorImg.setAttribute('src', '/images/animations/mail_error/200_once.gif'); // mailSendIndicatorImg.setAttribute('rel', 'preload');
