(function () {//для изоляции пространства имён
  document.addEventListener('DOMContentLoaded', function () {//действуем после загрузки DOM
    try {//чтобы скрипт не падал, для каждого блока желательно ставить try catch (синхронная конструкция)
      ...
      //}
    } catch (err) {//обрабатываем ошибки, если объект ошибки не нужен, мы можем пропустить его, используя 'catch {' вместо 'catch(err) {'  
      console.log('ОШИБКА: ');
      console.log(err);
    ...
    window.onerror = function (message, url, line, col, error) {//«глобальный» обработчик ошибок, чтобы ловить ошибки, которые «выпадают наружу»
      console.log(error);
      console.log(`${message}\n в ${line}:${col} на ${url}`);
    };
  });
